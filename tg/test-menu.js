...
(async () => {
  ...
  дано(await page.$$eval('#eatbody .mnrow', r => r.length) === 4, 'четыре приёма');
  дано(await page.$eval('#eatbody .mnrow', r => getComputedStyle(r).display) === 'block', 'строка приёма — блоком, не чужой сеткой замеров (снимок 24.09)');
