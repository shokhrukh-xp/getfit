...
(async () => {
  ...
  console.log('\n═══ 6.5 · разбор приёма: что было бы лучше ═══');
  /* 24.09, его просьба: по нажатию на запись — почему такая оценка и что было
     бы лучше. Числа (+25 г белка) считает код; тренер пишет только правку
     этого блюда. Проверяем живую модель: правка есть, своих чисел белка,
     жира и калорий в ней нет, второй раз ответ берётся из записи. */
  const ДР = 'svc_t_razbor', сег = new Date().toLocaleDateString('sv-SE', { timeZone: 'Asia/Tashkent' });
  await стереть(ДР);
  await анкета(ДР, { sex: 'm', age: 38, ht: 178, bw: 88.5, only: 'food', wt: 'down', gym: 'health' });
  const зп = await З('/meal/add', { uid: ДР, date: сег, kind: 'завтрак', text: 'Яичница из 4 яиц на сливочном масле с отрубным хлебом',
    kcal: 602, prot: 37, fat: 38, carb: 30, fib: 4, sug: 5 });
  дано(!!(зп && зп.id), 'приём записан' + sb_err(зп));
  const оц = await З('/day/score?uid=' + ДР + '&date=' + сег);
  const пр = ((оц && оц.приёмы) || []).find(x => x.id === зп.id) || {};
  дано((пр.разбор || []).length === 5, 'у приёма разбор по пяти пунктам: ' + (пр.разбор || []).map(x => x.имя + (x.v > 0 ? ' −' + x.v : x.ok ? ' ✓' : ' —')).join(', '));
  const лч = await З('/meal/better', { uid: ДР, id: зп.id });
  const сс = (лч && лч.советы) || [];
  сс.forEach(x => console.log('    ' + x.голова + ': ' + x.как + (x.тренер ? '' : '  (правило)')));
  дано(сс.length === (пр.why || []).length && сс.length > 0, 'по совету на каждый вычет: ' + сс.length);
  дано(сс.some(x => x.тренер), 'хотя бы одну правку написал тренер, а не правило' + (лч && лч.запас ? ' (модель не ответила)' : ''));
  const своиЧисла = сс.filter(x => x.тренер && /\d+\s*(ккал|г\s*(белка|жира|клетчатки|сахара))/.test(x.как));
  дано(!своиЧисла.length, 'в правках тренера нет своих чисел нутриентов' + (своиЧисла.length ? ': ' + своиЧисла.map(x => x.как).join(' | ') : ''));
  const лч2 = await З('/meal/better', { uid: ДР, id: зп.id });
  дано(!!(лч2 && лч2.кеш) || !!(лч && лч.запас), 'второй раз — из записи, без модели');
  await стереть(ДР);
