...
(async () => {
  ...
  дано(/ушло 4,0 кг/.test(вывод) && /По оценке весов, жиром — 2,1/.test(вывод), 'вывод сверху и он про жир: ' + вывод);
  дано(/нельзя судить о сохранности мышц/.test(await текст(page, '.srv p')), 'и сказано, что с мышцами: ' + await текст(page, '.srv p'));
  ...
  дано(/InBody/.test(await текст(page, '.srsrc')), 'источник на месте: ' + (await текст(page, '.srsrc')).slice(0, 40));
