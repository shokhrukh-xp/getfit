...
(async () => {
  ...
  /* Быстрый прогон (tg/run-fast.js) делит экраны между потоками: с GF_PART=k/n
     этот процесс обходит только экраны с номером k по модулю n. Перед каждым
     нажатием экран и так открывается заново, так что части не зависят друг от
     друга. Порог «нажато» — по доле экранов: целиком их больше 40. */
  const ЧАСТЬ = (process.env.GF_PART || '').match(/^(\d+)\/(\d+)$/);
  const ВСЕ_ЭКРАНЫ = [['home', null], ['gym', null], ['gym', 'Журнал'], ['gym', 'Каталог'],
                  ['food', null], ['food', 'Рейтинг']];
  const ЭКРАНЫ = ВСЕ_ЭКРАНЫ.filter((_, i) => !ЧАСТЬ || i % +ЧАСТЬ[2] === +ЧАСТЬ[1]);
  const ПОРОГ = ЧАСТЬ ? ЭКРАНЫ.length : 40;
  ...
  console.log('');
  дано(нажато > ПОРОГ, 'нажато кнопок: ' + нажато + (ЧАСТЬ ? ' (экраны: ' + ЭКРАНЫ.map(э => э[0] + (э[1] ? ' · ' + э[1] : '')).join(', ') + ')' : ''));
