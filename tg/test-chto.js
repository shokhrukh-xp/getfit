...
(async () => {
  ...
  /* ── середина дня: завтрак съеден, впереди обед и ужин ── */
  ...
  дано(/571/.test(цель) && /72/.test(цель), 'показана цель ОДНОГО приёма, а не остаток дня 849: ' + цель);

  /* ── раскладка: приёмы впереди и что достанется каждому ── */
  const чипы = await page.$$eval('.podraz button', bs => bs.map(b => b.innerText.replace(/\s+/g, ' ').trim()));
  дано(чипы.length === 2, 'показаны только основные приёмы впереди: ' + чипы.join(' | '));
  дано(чипы.join(' ').indexOf('перекус') < 0, 'перекус заранее не планируется: ' + чипы.join(' | '));
  дано(/обед\s*571/.test(чипы[0]) && /ужин\s*278/.test(чипы[1]),
  ...
  дано(/67%/.test(под) && /849/.test(под), 'сказано, какая это доля от остатка дня: ' + под);
  дано(/по твоим 14 дням/.test(под), 'и откуда взята доля: ' + под);
  ...
  дано(строки.length >= 8, 'каталог показывает прошлые записи и базу вместе: ' + строки.length);
  дано(строки[0] === 'Творог, яйца, греческий йогурт и банан',
    'сверху то, что лучше ложится в цель приёма: ' + строки[0]);
  const ярлык = await page.$$eval('.eatrow .en s', ss => ss.map(x => x.innerText));
  /* «Своё» было неточным: это прошлые записи, а еда в них могла быть и из
     доставки, и из заведения. Его замечание 17.09. */
  дано(ярлык.indexOf('из дневника') >= 0, 'прошлые записи подписаны «из дневника»: ' + ярлык.slice(0, 3).join(' | '));
  дано(ярлык.indexOf('своё') < 0, 'и слова «своё» на экране больше нет');
  дано(ярлык.indexOf('японская') >= 0 || ярлык.indexOf('итальянская') >= 0, 'у блюд базы подписана кухня');
  ...
  дано(/278/.test(цель2), 'выбор другого приёма меняет цель: ' + цель2);
  ...
  /* ── «Посмотреть» вместо «Записать» ──
     Каталог — выбор ДО еды. Обе кнопки есть и у прошлых записей: еда оттуда
     могла быть из заведения, и «Где поесть» для неё осмысленно. */
  ...
  дано(!!(await page.$('.eatopen [data-catmap]')), 'у прошлой записи «Где поесть» тоже есть');
  ...
  let сс = await ссылки();
  дано(сс.length === 2 && /google\.com\/search/.test(сс[0]) && /yandex\.uz\/maps/.test(сс[1]),
    'одна ведёт в поиск, другая на карты: ' + сс.join(' | ').slice(0, 120));
  дано(decodeURIComponent(сс[0]).indexOf('Творог, яйца, греческий йогурт и банан') > 0,
    'запись без порции в названии уходит целиком, а не до первой запятой: ' + decodeURIComponent(сс[0] || ''));
  ...
  await page.click('.eatopen [data-catmap]'); await page.waitForTimeout(400);
  сс = await ссылки();
  дано(сс.length === 4, 'обе кнопки сработали: ' + сс.length);
  дано(/google\.com\/search/.test(сс[2]) && decodeURIComponent(сс[2]).indexOf('Роллы с лососем') > 0
       && decodeURIComponent(сс[2]).indexOf('6 шт') < 0,
    'в поиск уходит блюдо без порции: ' + decodeURIComponent(сс[2] || ''));
  дано(/yandex\.uz\/maps/.test(сс[3]) && decodeURIComponent(сс[3]).indexOf('Роллы с лососем') > 0,
    '«Где поесть» ведёт на карты с тем же запросом: ' + decodeURIComponent(сс[3] || ''));
  ...
  /* «полпорции» — это про размер, а не про еду, и в запрос идти не должно */
  await page.fill('#eatq', 'полпорц'); await page.waitForTimeout(500);
  await page.click('.eatopen [data-catmap]'); await page.waitForTimeout(400);
  сс = await ссылки();
  const посл = decodeURIComponent(сс[сс.length - 1] || '');
  дано(посл.indexOf('Плов с говядиной, салат, лепёшка') > 0 && посл.indexOf('полпорц') < 0,
    'размер порции из запроса вырезан, остальное цело: ' + посл);
  await page.close();
  ...
  await M.поднять(page, { theme: 'dark', wait: 2800, eat: {
    'тип': 'завтрак', 'впереди': 3, 'пусто': false, 'всё': false, 'безНормы': false,
    'ост': { kcal: 1850, prot: 140, fib: 25 }, 'цель': { kcal: 518, prot: 39, fib: 7 },
    'раскладка': [{ kind: 'завтрак', kcal: 518, prot: 39, fib: 7, 'доля': 28 },
                  { kind: 'обед', kcal: 721, prot: 55, fib: 9.8, 'доля': 39 },
                  { kind: 'ужин', kcal: 611, prot: 46, fib: 8.3, 'доля': 33 }],
    'доли': { 'завтрак': 0.28, 'обед': 0.39, 'ужин': 0.33 },
  ...
  дано(/518/.test(нов), 'и цель приёма всё равно посчитана: ' + нов.slice(0, 120));
  const чипыН = await page.$$eval('.podraz button', bs => bs.map(b => b.innerText.replace(/\s+/g, ' ').trim()));
  дано(чипыН.length === 3, 'с утра впереди три основных приёма: ' + чипыН.join(' | '));
  дано(чипыН.join(' ').indexOf('перекус') < 0, 'и перекуса среди них нет: ' + чипыН.join(' | '));
  ...
  /* ── основных приёмов впереди не осталось: вот это и есть перекус ── */
  page = await br.newPage({ viewport: { width: 390, height: 900 } });
  await page.route('**://cdn.jsdelivr.net/**', r => r.abort().catch(() => {}));
  await M.поднять(page, { theme: 'dark', wait: 2800, eat: {
    'тип': 'перекус', 'впереди': 1, 'пусто': false, 'всё': false, 'безНормы': false,
    'ост': { kcal: 320, prot: 28, fib: 4 }, 'цель': { kcal: 320, prot: 28, fib: 4 },
    'раскладка': [{ kind: 'перекус', kcal: 320, prot: 28, fib: 4, 'доля': 100 }],
    'доли': { 'завтрак': 0.3, 'обед': 0.47, 'ужин': 0.229 },
    'дней': 14, 'свои': true, 'откуда': 'по твоим 14 дням',
    'плотность': { 'надо': 8.8, 'слово': 'без белкового блюда не обойтись', 'потолок': 22,
                   'шкала': [[8, 'подойдёт любая обычная еда'], [12, 'без белкового блюда не обойтись'],
                             [9999, 'нет']], 'пусто': 'белок уже закрыт' },
    'норма': { kcal: 1550, prot: 160, fib: 20 }, 'съел': { kcal: 1230, prot: 132, fib: 16 },
    'варианты': [], 'примерка': [] } });
  await вкладка(page);
  const пк = await page.$eval('#fpod', e => e.innerText.replace(/\s+/g, ' ')).catch(() => '');
  дано(/320/.test(пк) && /За день осталось 320/.test(пк),
    'последнему приёму достаётся весь остаток, без долей: ' + пк.slice(0, 120));
  дано((await page.$$eval('.podraz button', bs => bs.length)) === 0,
    'переключать нечего — чипов нет');
  дано((await page.$$eval('.eatrow', rs => rs.filter(r => r.classList.contains('brk')).length)) === 0,
    'и ломать больше нечего: после последнего приёма дня не остаётся');
  await page.close();
