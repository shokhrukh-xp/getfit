...
async function поднять(page, o) {
  ...
  await page.route('**/getfit-sync.sh-pulatov.workers.dev/**', async route => {
    ...
    if (p === '/eat') {
      ...
      /* Середина дня: завтрак съеден, впереди обед, ужин и перекус. Числа —
         его собственный день 17.09, на котором мы эту механику и делали. */
      ...
      return дать({ ok: true, тип: 'обед', впереди: 3, пусто: false, 'всё': false, 'безНормы': false,
        ост: { kcal: 849, prot: 107, fib: 15 }, 'цель': { kcal: 486, prot: 61, fib: 8.6 },
        'раскладка': [{ kind: 'обед', kcal: 486, prot: 61, fib: 8.6, 'доля': 57 },
                      { kind: 'ужин', kcal: 237, prot: 30, fib: 4.2, 'доля': 28 },
                      { kind: 'перекус', kcal: 126, prot: 16, fib: 2.2, 'доля': 15 }],
        'доли': { 'завтрак': 0.266, 'обед': 0.419, 'ужин': 0.205, 'перекус': 0.109 },
        'дней': 15, 'свои': true, 'откуда': 'по твоим 15 дням',
        ...
        варианты: было.slice(0, 3).map(v => Object.assign({}, v, {
          как: 'в калории попадает, белка не хватит ' + Math.max(0, 61 - v.prot) + ' г.',
